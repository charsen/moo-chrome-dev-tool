...
describe('listUsers — v0.8.9 tier-3 建单页兜底（普通账号 users 权限墙）', () => {
  ...
  it('v2 200 + 空 users 数组 → 也试 tier-3（部分实例对无权限账号返空列表而非 4xx）', async () => {
    const { listUsers } = await import('@/background/zentao/client')
    let v1Hit = 0
    vi.stubGlobal('fetch', vi.fn(async (url: string) => {
      if (url.includes('/users/login')) {
      ...
      if (url.includes('/api.php/v2/users')) {
        return mockJsonRes({ users: [] })  // 200 + 空列表（权限墙的另一种形态）
      }
      if (url.includes('/api.php/v1/users')) { v1Hit++; return mockJsonRes({ users: [] }) }
      if (isBugCreatePageUrl(url)) {
        return bugCreatePageRes({ users: { zhangsan: '张三' }, moduleOptionMenu: {} })
      }
      return mockJsonRes({})
    }))
    const r = await listUsers(env, 200, 14)
    ...
    // v2 空列表直接进 tier-3，不需要再撞 v1
    expect(v1Hit).toBe(0)
  })

  it('空列表 + tier-3 也空/失败 → 原样返 ok 空列表（兼容真没用户可显示的边角）', async () => {
    const { listUsers } = await import('@/background/zentao/client')
    vi.stubGlobal('fetch', vi.fn(async (url: string) => {
      if (url.includes('/users/login')) {
        return mockJsonRes({ status: 'success', token: 't', user: { id: 1, account: 'a', realname: 'A' } })
      }
      if (url.includes('/api.php/v2/users')) return mockJsonRes({ users: [] })
      if (isBugCreatePageUrl(url)) return new Response('login page', { status: 200 })  // 非 JSON → tier-3 失败
      return mockJsonRes({})
    }))
    const r = await listUsers(env, 200, 14)
    expect(r.ok).toBe(true)
...
// ─────────────────────────── ensureCookieSession fresh（v0.8.9 Fix G）───────────────────────────
// fresh=true（SubmitDialog 2 分钟复查路径）必须先 _clearToken 强制真 login —— 否则
// SW 长存活时 token 缓存常驻，复查永远零网络空转返「✓ 已登录」，session 服务端早过期
// 也骗用户（提交时附件才挂）。首检不带 fresh 保留缓存快路径。
describe('ensureCookieSession — v0.8.9 fresh 真探测（Fix G）', () => {
  const loginRes = () =>
  ...
  it('① fresh=true：暖缓存下 login fetch 真发生（强制真探测）', async () => {
    ...
    // fresh=true → 必须再发一次真 login（修前：吃暖缓存 0 fetch 空转）
    const r = await ensureCookieSession(env, { fresh: true })
    expect(r.ok).toBe(true)
    if (r.ok) expect(r.data.realname).toBe('爱丽丝')
    expect(fetchMock).toHaveBeenCalledTimes(2)
    expect(String(fetchMock.mock.calls[1]?.[0])).toContain('/users/login')
  })
  ...
  it('② 不带 fresh：暖缓存 0 新增 fetch（快路径不回归）', async () => {
    ...
    const r = await ensureCookieSession(env)            // 无 fresh
    expect(r.ok).toBe(true)
    if (r.ok) expect(r.data.realname).toBe('爱丽丝')
    expect(fetchMock).toHaveBeenCalledTimes(1)          // 零新增网络请求
  })
