...
>
> **当前覆盖数字看 `HANDOFF.md` 顶部**（本文写过的具体数字会随版本漂移，不再每次回填）。历史基线参考：v0.4.5 时 356 单测 + 100 e2e。
>
...
> ⚠️ **以下数字为 v0.1.x 时代基线**（vitest 161 + E2E 34 = 195 case）。**当前最新数字看 `HANDOFF.md`**。下面 P0/P1 优先级分析自此未回填，仅作历史路线图 reference。当前测试策略走 schema fuzz / 真实 fixture / 双 MCP 分断面，跟此表结构无直接 1:1 映射。

**v0.1.x 当时**：已 ✓ 195 case（vitest 161 + E2E 34）。**枚举出 ⏳ ~ 600+，真值优先级排序后：**
