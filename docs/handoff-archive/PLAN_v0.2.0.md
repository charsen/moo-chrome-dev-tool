...
> ⚠️ **本文档为已完成的归档计划**（v0.2.0 → v0.2.3 → v0.4.x 演进已完整 land）。所有
> TODO / 接力清单 / 「下次会话开干」字样仅作历史 reference，**不再是活档**。当前生效的
> 禅道集成实现见 `src/background/zentao/{client,submit}.ts` + `docs/ZENTAO_SETUP.md`。
>
> 2026-05-21 探测会话产出 +（同日）P1 接手会话事实纠正。
> 测试用禅道实例：https://z.example.com（实际 dogfood 用自己的实例）
> 测试用项目 ID 26（v0.2.0 立项时的 fixture，是否仍在用以 `tests/` 当前实际为准）
