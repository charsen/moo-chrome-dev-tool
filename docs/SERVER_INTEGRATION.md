...
> 字段名 `token` 是扩展默认 Payload 模板里写的——开发者可以改模板把它放到任何路径（比如 `auth.token`），你的接口跟着改即可。
...
**大视频场景**：扩展可以切到 `multipart/form-data`，截图和录屏走二进制文件字段（省 33% 体积）。你按 `Content-Type` 分支处理：multipart 时从 file 字段读二进制（`token` 仍是普通 form 字段），转回 base64 跟 JSON 模式对齐。
