...
```jsonc
{
  ...
  "title": "登录按钮点了没反应",              // 必填
  "description": "复现步骤...\n\n页面: ...",
  "screenshot": "data:image/png;base64,iVBOR...",       // 首图（= screenshots[0]，向后兼容）
  "screenshots": [ "data:image/png;base64,...", ... ],  // v0.8.11 起：全部截图数组（含首图，上限 5）
...
> **多张截图（v0.8.10 起，v0.8.11 修默认模板）**：
> - **JSON 模式**：默认 Payload 模板自带 `"screenshots": {{imagesJson}}`（全部截图的 base64 数组，含首图），服务端读 `screenshots[]` 逐张落盘即可。`screenshot` 仍是首图、向后兼容。⚠ v0.8.10 默认模板漏了 `{{imagesJson}}` → 开箱只发首图；v0.8.11 起默认模板已补、老配置经 `migrateServerTemplate` 自动升级。
> - **multipart 模式**：第一张走配置的图片字段名（如 `screenshot`），第 2 张起追加 `screenshot_2`、`screenshot_3` …（文件名 `screenshot_N.png`，上限 5 张）。
> - 不认识这些字段的老服务端忽略即可，无需改动；要收多图按上述约定取。
