...
把网页 bug 一键提到禅道。**不用截图传文件夹、不用粘 URL、不用手填环境信息**，点页面右下角悬浮球的截图按钮（或按快捷键 `⌘⇧B / Ctrl+Shift+B`），填个标题，bug 就进禅道了，自动带：
...
**建议用最新版**（禅道兼容 / 安全 / 体验问题每个版本都在持续修，老版本可能踩已修过的坑）。[📦 拿最新版（gitee releases）](https://gitee.com/charsen/moo-chrome-dev-tool/releases) — 永远去这个页面拉**最新的那条**（页面顶部第一条就是 latest，不要按下面老版本链接下载）。
...
装好后浏览器右上角能看到 Moo 图标。**点一下这个图标，在弹窗里打开「允许向上报服务器发送请求」开关——不开这步，悬浮球不会出现。**
...
1. 在你正在调 bug 的网页上 → 找悬浮球（默认在**右下角**，是个横向小条 `[Moo][📷][🎬]`，3 个图标：拖动手柄 / 截图 / 录屏）→ 点中间「截图」按钮。也可以直接按快捷键 `⌘⇧B / Ctrl+Shift+B`
...
A：先检查浏览器右上角有没有 Moo 图标。没有就回第 1 步把扩展装好。有图标但悬浮球不出现，按顺序查：
- 点 Moo 图标看弹窗：如果提示「『向上报服务器发送请求』未授权 — 悬浮球不会出现」，点「一键启用」（**最常见原因**）
- 检查你访问的网页 URL 是不是匹配 Moo 项目配置的「URL 匹配」规则（环境 Tab 看）。不匹配就不显示
- 弹窗里「悬浮球」开关是不是被关了（之前可能临时隐藏过当前站点），打开它
- 球只是被页面挡住的话，直接按 `⌘⇧B / Ctrl+Shift+B` 截图，不用找球
...
- 或者直接按 `⌘⇧B / Ctrl+Shift+B` 快捷键截图 —— 完全不用碰悬浮球

### Q：提交后禅道里没看到截图？

A：v0.2.3 起 Moo 用你「环境」里配的账号密码自动登录禅道，正常情况下截图都能上去。
...
打开历史 Tab 时，所有有单号的记录（禅道和 webhook 都算）会自动同步一次状态。想强制刷新：点历史 Tab 右上角的「同步远端状态」按钮。
