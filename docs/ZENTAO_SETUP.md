...
**建议用最新版**（v0.4.x 系列在禅道兼容 + 安全 + UX 三个方向持续加固，老版本可能踩已修过的坑）。[📦 拿最新版（gitee releases）](https://gitee.com/charsen/moo-chrome-dev-tool/releases) — 永远去这个页面拉**最新的那条**（页面顶部第一条就是 latest，不要按下面老版本链接下载）。
