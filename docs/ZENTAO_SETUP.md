...
适用：Moo v0.2.0+（[下载 v0.2.0 zip](https://gitee.com/charsen/moo-chrome-dev-tool/releases/download/v0.2.0/moo-chrome-dev-tool-0.2.0.zip)）
...
[下载 zip](https://gitee.com/charsen/moo-chrome-dev-tool/releases/download/v0.2.0/moo-chrome-dev-tool-0.2.0.zip) → 解压
...
**这一步千万别跳。** Moo 提 bug 借用的是「你已经登录禅道」这个状态。

浏览器打开 `https://你公司的禅道.chandao.net` → 输账号密码登录 → 别关掉。

> **以后每天上班开浏览器后，要先去禅道页面登一次**，Moo 才能正常提 bug。登录禅道之后那个 tab 可以关掉，登录状态在浏览器里还能保留半天到一天。
...
- 📸 截图（直接渲染出来，不用下载）
- 🎥 录像（如果你录了，给个下载链接）
- 🌐 网络请求 curl 代码（看哪些请求出错；要复现请求点最下面的 `moo-requests.curl.sh` 附件下载）
...
A：99% 是**忘了先登录禅道**了。Moo 的截图必须靠「你已登录禅道」才能上传到禅道。

打开 SubmitDialog 时左侧上方有个状态条：

- ✅ **绿色「✓ 已登录禅道（你的名字）」**——OK 可以提
- ❌ **红色「⚠ 未登录禅道」**——点右边「打开禅道登录」按钮 → 在新标签登录 → 回来点「重新检查」

### Q：提示「提交成功」但禅道里 bug 标记成「由 system 创建」？

A：可能你升到 v0.2.0 之前装过 Moo 的老版本（v0.1.x）。卸载老的、装 [v0.2.0 zip](https://gitee.com/charsen/moo-chrome-dev-tool/releases/download/v0.2.0/moo-chrome-dev-tool-0.2.0.zip)。v0.2.0 修复了这个问题，正常会显示你的真名。
