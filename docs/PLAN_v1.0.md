...
>
> **v0.8.x 回看**：多条建议已落地（v0.6.0 权限最小化、v0.7.0 动态注册、CWS 物料 `docs/cws/` 就绪待提审）。本文不逐条回填，当前状态以 `HANDOFF.md` 为准。
...
按用户偏好「步伐大一点 + 多更新些再发」+ 4 波 agent review 闭环节奏，**v0.6.x + v0.7.0 一周内**把上面 6 个 minor 的代码层全部做完：

| 原计划版本 | 原排期工作 | 实际版本 | 实际完成情况 |
|---|---|---|---|
| v0.6.0 | P0 background MessageRouter | v0.6.0 | ✅ 1000 → 254 行，6 handler 文件 + 52 单测 |
| v0.7.0 | P1 Environment 拆 + IssueAdapter interface | **v0.6.0** | ✅ Environment 1206 → 582 + IssueAdapter 完整实装（4 adapter 文件 + dispatch 闭环）|
| v0.8.0 | P2 SubmitDialog 拆 + P3 retryQueue 多轨 | v0.6.0（P3）/ **v0.7.0**（P2）| ✅ retryQueue adapter dispatch；P2 SubmitDialog 拆见 v0.7.0 |
| v0.9.0 | `<all_urls>` 改 optional + 隐私政策 + CWS 物料 | **v0.6.0** + **v0.7.0** | ✅ #128 host_permissions optional + content_scripts 动态注册 + docs/cws/ 物料 |
| v1.0.0 | CWS 上架 | 进行中 | 🟡 用户已登录开发者平台，等截图 + 后台填表 + 评审 |

**dogfood 实测损失** = v0.6.2 dogfood 同事撞 1 个 v1 endpoint 403 bug（已 hotfix）+ v0.6.1 silent 回归 11 个 e2e fail 跨 2 minor 没人发现（v0.6.3 复盘修了）。损失可控。

**剩 v0.7.1+ 工作**：
- 自托管 telemetry collector（PLAN 决策 5）— P1
- web_accessible_resources `<all_urls>` 收窄 — 中 ROI（CWS 评审风险点）
- Environment matchPatterns 实时校验 UI — 小改 UX 优化
- i18n 全量迁移 — 延后 v1.2+（PLAN 决策 5 已确认）

---

**注**：本文档由 Plan agent v0.5.1 自动生成 + 2026-05-25 v0.7.0 阶段性更新。原计划假设 6-9 月 / 4 波 minor，实际通过 4 波 agent review 闭环节奏一周做完。后续节奏建议：CWS 上架等待期间慢下来；上架后转「每 feature PR 1 波专项 agent review + 月度全量」。
