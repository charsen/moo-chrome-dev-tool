 ...
 *   - UX11 dialog 内按下→拖到 mask 灰区松手：不关窗、已填内容保留（v0.8.16 maskClosable=false 修误关）
 *
 * 不覆盖（real-flow spec 负责）：跨「再截一张」卸载重挂的位置记忆 + 无闪跳
 * （--moved animation:none）+ 真 closed shadow / 真宿主页交互。
 * mask 直点防误关（不 cancel + bump 脉冲）已有 dialog-submit D3 锁，不重复。
 */
...
test('UX6 · 按住缩小/×按钮拖动：不触发整窗拖拽，也不误触按钮（拖离后松手 click 不成立）', async ({ context, extensionId, sw }) => {
  ...
  // 注：press 在 dialog 内 + release 落 mask 时浏览器把 click 派发到共同祖先（= mask），
  //   曾被 @click.self 误判成「点 mask」→ cancel（v0.8.16 前上报的 bug）。现在
  //   SubmitDialog 传 maskClosable=false，mask 点击不再 cancel —— 跨界松手场景由
  //   UX11 专锁。本 case 落点仍留在 dialog 内，聚焦「按钮上按下拖动」这一件事。
...
test('UX11 · dialog 内按下拖到 mask 灰区松手：弹窗不关、已填内容保留（maskClosable=false）', async ({ context, extensionId, sw }) => {
  const page = await openDialog(context, sw, extensionId)
  await page.locator('#moo-title').fill('拖拽选中文本时不许关我')

  // 用户真实踩坑姿势：在标题输入框里按下（想拖选文本），手一滑拖出弹窗、在 mask
  // 灰区松手 —— 浏览器把 click 派发到共同祖先（= mask），老行为 @click.self 误判
  // 「点 mask」→ cancel 丢掉已填内容。v0.8.16 maskClosable=false 修掉。
  const titleBox = await page.locator('#moo-title').boundingBox()
  if (!titleBox) throw new Error('title bbox missing')
  const maskBox = await page.locator('.moo-dialog-mask').boundingBox()
  if (!maskBox) throw new Error('mask bbox missing')

  await page.mouse.move(titleBox.x + titleBox.width / 2, titleBox.y + titleBox.height / 2)
  await page.mouse.down()
  // mask 左上 20,20 一定在 dialog 容器外（容器居中）
  await page.mouse.move(maskBox.x + 20, maskBox.y + 20, { steps: 5 })
  await page.mouse.up()
  await page.waitForTimeout(50)

  await expect(page.locator('.moo-dialog'), '跨界松手弹窗必须还在').toBeVisible()
  await expect(page.locator('#moo-title')).toHaveValue('拖拽选中文本时不许关我')
  const emits = await readEmits(page)
  expect(emits.some((e) => e.event === 'cancel'), '跨界松手不应 emit cancel').toBe(false)
})
