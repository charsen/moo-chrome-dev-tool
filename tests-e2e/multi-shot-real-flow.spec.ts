...
function makeConfig(imageFormat: 'base64' | 'multipart') {
  // v0.8.11：用真实默认模板的多图字段名 "screenshots":{{imagesJson}}（与 DEFAULT_PAYLOAD_TEMPLATE
  // + 云端 intake 契约一致），而非旧测试自造的 "images" —— 否则又是「测了没人用的配置」。
  const payloadTemplate = imageFormat === 'base64'
    ? '{"title":"{{title}}","screenshot":"{{image}}","screenshots":{{imagesJson}},"url":"{{url}}"}'
    : '{"title":"{{title}}"}'
  return {
    globalEnabled: true,
    projects: [{
...
// v0.8.12：「再截一张」改延迟触发 —— 点 addBtn 后弹窗收起，浮起 arming 触发器，
// 用户操作好页面再点「现在截图」才真截。
const isArmTrigger: Pred = (n, a) => (a['class'] ?? '').split(' ').includes('moo-arm-row')
const isShootBtn: Pred = (n, a) => n.nodeName === 'BUTTON' && (a['class'] ?? '').includes('moo-arm-btn--shoot')
const isArmCancelBtn: Pred = (n, a) => n.nodeName === 'BUTTON' && (a['class'] ?? '').includes('moo-arm-btn--cancel')
...
/** 「再截一张」延迟触发（v0.8.12）：点 addBtn → 验证 arming 触发器浮起（不立刻进 annotating）
 *  → 点「现在截图」→ 等 Annotator。dwell：模拟用户在 arming 态操作页面（可回调改页面/滚动）。 */
async function addShotViaArm(cdp: CDPSession, opts?: { duringArm?: () => Promise<void> }): Promise<void> {
  const addBtn = await waitFor(cdp, isAddShotBtn, '再截一张按钮')
  await cdpClick(cdp, addBtn)
  // 关键断言：点了不立刻截 —— 先出 arming 触发器，且此刻没有 Annotator
  await waitFor(cdp, isArmTrigger, 'arming「现在截图」触发器（点再截一张后应浮起而非立刻截）')
  expect(findAll(await snapshot(cdp), isAnnotator).length, 'arming 态不应已进入 annotating').toBe(0)
  if (opts?.duringArm) await opts.duringArm()
  const shootBtn = await waitFor(cdp, isShootBtn, 'arming「现在截图」按钮')
  await cdpClick(cdp, shootBtn)
  await waitFor(cdp, isAnnotator, 'arming 截图后的 Annotator', 12_000)
}
...
test('F1 · 真流程闭环：悬浮球→真截屏→标注→标题→再截一张→草稿+2缩略→提交→mock 收到 images[2]', async ({ context, sw }) => {
  ...
  // ── 「＋ 再截一张」→ arming 态（弹窗收起）→ 在 arming 期间改页面内容（模拟用户先操作
  //    页面：切 tab / 滚动 / 改色，保证两张截图像素不同）→ 点「现在截图」→ 真截屏 ──
  const addBtn = await waitFor(cdp, isAddShotBtn, '再截一张按钮')
  expect(textOf(addBtn)).toContain('（1/5）')
  await addShotViaArm(cdp, {
    duringArm: async () => {
      // arming 态弹窗已收起，标题输入框应不在（验证「弹窗收起」语义）
      expect(findAll(await snapshot(cdp), isTitleInput).length, 'arming 态弹窗应已收起').toBe(0)
      await page.evaluate(() => {
  ...
  const body = JSON.parse(received[0]!.body.toString('utf8')) as { title: string; screenshot: string; screenshots: string[]; url: string }
  expect(received[0]!.contentType).toContain('application/json')
  expect(body.title).toBe(TITLE)
  expect(body.url).toContain(`http://127.0.0.1:${PORT}/`)
  // v0.8.14：webhook/cloud 路径上传前把图重编码成 WebP（治复杂截图超 8MB 静默丢）。
  // 故 body 里是 data:image/webp，不再是 PNG（单测 webhookAdapter.test.ts 同款断言）。
  expect(body.screenshot.startsWith('data:image/webp;base64,')).toBe(true)
  expect(Array.isArray(body.screenshots)).toBe(true)
  expect(body.screenshots).toHaveLength(2)
  expect(body.screenshots[0], '约定 screenshot === screenshots[0]').toBe(body.screenshot)
  expect(body.screenshots[1]!.startsWith('data:image/webp;base64,')).toBe(true)
  expect(body.screenshots[1], '两张截图内容应不同（页面已改色 + 仅第 1 张画了矩形）').not.toBe(body.screenshots[0])
...
test('F2 · multipart 闭环：2 张截图 → form-data 收到 screenshot + screenshot_2 字段', async ({ context, sw }) => {
  ...
  await addShotViaArm(cdp, {
    duringArm: async () => { await page.evaluate(() => { document.body.style.background = '#86efac' }) }
  })
  await annotateAndNext(page, cdp, false)
...
// ═══════════════════════════════════════════════════════════════════════════
// F5 · arming 态「取消」（v0.8.12）：点再截一张 → 触发器浮起 → 点取消 → 回弹窗，
//      不新增截图、标题草稿保留、再截一张按钮计数不变
// ═══════════════════════════════════════════════════════════════════════════
test('F5 · arming 取消：触发器浮起→取消→回弹窗，shot 不变 + 草稿在', async ({ context, sw }) => {
  test.setTimeout(90_000)
  const { page, cdp } = await openMatchedPage(context, sw, 'base64')
  ...
  await captureOnce(cdp)
  await annotateAndNext(page, cdp, false)
  const TITLE = 'arming 取消应保留这条草稿'
  await page.waitForTimeout(700)
  await page.keyboard.type(TITLE)
  expect(await thumbCount(cdp), '取消前应有 1 张').toBe(1)

  // 点「再截一张」→ arming 触发器浮起 + 弹窗收起
  const addBtn = await waitFor(cdp, isAddShotBtn, '再截一张按钮')
  await cdpClick(cdp, addBtn)
  await waitFor(cdp, isArmTrigger, 'arming 触发器')
  await waitGone(cdp, isTitleInput, 'arming 态弹窗应收起')

  // 点 arming「取消」→ 弹窗重挂
  const cancelBtn = await waitFor(cdp, isArmCancelBtn, 'arming 取消按钮')
  await cdpClick(cdp, cancelBtn)
  await waitGone(cdp, isArmTrigger, '取消后 arming 触发器应消失')

  // 核心断言：回弹窗、标题草稿还在、缩略仍 1 张、计数仍 1/5
  const titleNode = await waitFor(cdp, isTitleInput, '取消后重挂的标题输入框')
  expect(await cdpValue(cdp, titleNode), 'arming 取消应保留标题草稿').toBe(TITLE)
  expect(await thumbCount(cdp), 'arming 取消不应新增截图').toBe(1)
  const addBtn2 = await waitFor(cdp, isAddShotBtn, '再截一张按钮（计数应仍 1/5）')
  expect(textOf(addBtn2)).toContain('（1/5）')

  expect(received.length, 'arming 取消全程无上报').toBe(0)
  await page.close()
})
...
test('F4 · 位置记忆：拖开 → 再截一张重挂回原位（--moved + 无入场动画 = 无闪跳）；Esc 取消重开 → 居中复位', async ({ context, sw }) => {
  ...
  // ── 再截一张 → arming → 现在截图 → 真卸载（draft 存 dialogPos）→ 第 2 轮标注 → 重挂 ──
  await addShotViaArm(cdp)
  await annotateAndNext(page, cdp, false)
