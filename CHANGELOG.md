...
- 根目录长期经验与待办文件统一为 `NOTES.md`、`TODOS.md`，同步修正仓库内引用。
- 无扩展运行时、权限、上报或接口行为变化。
...
### 🔴 根因：复杂截图 2560px PNG 仍 >8MB → 服务端静默丢

v0.8.13 降采样只缩宽度、保留 PNG（无损）。复杂截图（满屏文字/细节）的 2560px PNG 仍可达 10–13MB，超云端 `extractBinary` 8MB/张上限 → **静默 skip 不建附件、请求仍 200**。用户截 3 张、2 张超限被丢 → cloud 只存 1 张。history 列表只显封面图，所以「history 也只 1 张」是列表设计，不是证据。lab-tester 真 cloud 实锤。

### 🛠 修复：上传前按目标有损重编码（治本）

截图入口仍降采样 2560px PNG（给标注/预览/history 用）；**格式重编码放在各 adapter 上传前**（目标已知 + 能给禅道正确文件名）：

- **webhook/cloud → WebP q0.9**（最清晰，cloud MIME 白名单含 webp）；**禅道 → JPEG q0.9**（老版本禅道不一定支持 webp，jpeg 通吃 + 文件名同步 `.jpg`）。JPEG 先铺白底防透明区变黑。
- 真实视口截图从 ~10MB PNG 压到 **~0.27MB**，8MB 闸再也撞不到。history 走原 req（PNG，thumbnailize 自管），用户回看无损。
- 失败兜底返原图（宁可大也别丢截图）。lab-tester 真 cloud 验：3 张大图全落库（对照 PNG 路径落 0 张）。

### 🛠 配套：迁移正则补漏 + 服务端兜底（moo-scaffold-cloud）

- **`insertScreenshotsField` 正则补「screenshot 末字段无尾逗号」**：旧正则强制要逗号，漏了 cloud 单图模板这种结构 → 那批用户多图迁移一直被跳过（另一类「只存 1 张」根因）。改尾逗号可选，两种结构都产出合法 JSON。
- **服务端 `ImageDownscaler::fit` + 降采样移到 8MB 检查前**：>8MB 的图不再静默丢，而是先缩宽、仍超限再**有损重压**到装得下。两端双保险（老客户端/极端图也不丢）。

**852 单测（扩展，含 reencode 17 例）+ 截图 e2e + type-check + build 全绿；服务端 443 测试全绿（含 fit 9 例 + >8MB 重压不丢集成测试）。**

### 发版决策小记（跳 RELEASE_TEST_CHECKLIST 理由）

非 BREAKING（不碰存储 schema / 上传协议：仅在各 adapter 上传前按目标有损重编码、失败兜底返原图；迁移正则放宽尾逗号只多覆盖一类合法结构）+ 全绿（852 单测含 reencode 17 例 + 181 e2e + type-check + build；服务端 443 测试全绿）+ 用户明示「发版」放行。dogfood 不足（重编码刚做）—— 跳过 checklist。lab-tester 已真 cloud 实锤：3 张大图全落库（对照 PNG 路径落 0 张）。F1 e2e 断言已对齐 v0.8.14 设计（上传前重编码 WebP，不再 PNG），multi-shot 5 条全绿；MS7 已知 flaky（不碰截图编码链路），单跑过。留 dogfood 观察点：① 复杂大图截多张全落库不再静默丢；② webhook/cloud 落 WebP、禅道落 JPEG（文件名 `.jpg`、白底防透明变黑）；③ history 回看仍 PNG 无损；④ 旧多图模板用户迁移不再被跳过。服务端 moo-scaffold-cloud 不在本次发版范围（独立 repo，测试已 push，用户自行 deploy）。

## v0.8.13

2026-06-18。无 BREAKING —— **截图上传前降采样到 ≤2560 宽**。

### 🗜 截图上传前瘦身

高 DPI / retina 屏 `captureVisibleTab` 截的是物理像素，1280 CSS 宽在 DPR 2–3 下实际 2560–3840px、PNG 5–6MB。新增 `downscaleToMaxWidth`，在 **SW 截图 handler 截完即缩**（单一收口点）→ 标注 / 预览 / 上传 / history 全拿到缩好的图，省上传带宽 + 减小 payload（缓解 webhook/zentao 重试队列 1MB 上限 + 多图叠加体积）。

- 与服务端 `ImageDownscaler::toMaxWidth` **同语义**：只缩不放（≤2560 原样返回）、高度按比例、保留格式（PNG→PNG 文字清晰 + alpha）、失败返原图不丢截图。
- 两端配合（防御纵深）：扩展先缩 → 上传即小图 → 服务端见 ≤2560 不重复处理；老版本/其它客户端上传 >2560 时服务端兜底缩。
- 配套服务端（moo-scaffold-cloud）补 `ImageDownscaler` 单测 + intake 集成测试（>2560 图落盘宽度 ≤2560）。
...
### ⚠ 多匹配时目标项目可见（修静默 default 提错）

一个页面 URL 可能命中多个 Moo 项目（matchPatterns 重叠）。此前快捷键/录屏路径多匹配时**静默 default 到第一个项目，全程无提示** → 容易提到错项目/错禅道（用户反馈的「提报到无项目」根因之一）。修复：

- **提交弹窗顶部警告条**（`matchCount > 1` 时）：「⚠ 本页命中 N 个项目，当前提交到「项目名」· 禅道 #id。提错可在悬浮球切换项目后重提」—— 禅道项目连 projectId 一起核对。
- **录制条目标标识**：录屏时显「→ 项目名」，录制中就能发现提错及时取消。
- 唯一匹配不显示，零打扰。不碰匹配引擎 / 存储 schema / 悬浮球强制选逻辑，零迁移。

### ✨ 「再截一张」改延迟触发（先切页再截）

应对「SPA 应用内切到另一个 tab 页再截图」的需求。点「＋ 再截一张」不再立刻截：

- 弹窗收起（草稿保留）+ 右下角浮起**可拖动**的「📷 现在截图」+「取消」。
- 用户自由操作页面（切 SPA tab / 滚动 / 展开面板）后点「现在截图」才真正截 → 标注 → 回弹窗（+1 张）。
- 新增 `arming` 状态 + `ArmShotTrigger.vue`；拖拽照搬悬浮球 pointer-capture，listener 四渠道 idempotent 收口 + onBeforeUnmount 兜底，无裸挂泄漏。
- 初次截图、单张「重新截图」不变（仍马上截）。captureVisibleTab 无 user-gesture 依赖，延迟触发更稳。
...
- **轻量遮罩**：去掉背景模糊，遮罩透明度降到 0.18（暗色页面翻浅雾）—— 录入 bug 时能看清弹窗后面的页面内容，不用来回切。
- **标题栏可拖拽**：按住标题栏拖动弹窗，四向防拖出视口；位置写进草稿，「再截一张」等跨重挂场景回来还在原地。
- **可缩小**：标题栏「—」把弹窗缩成右下角药丸「继续填写 Bug」，期间页面可正常滚动操作；点药丸或 Esc 恢复，焦点回到弹窗。

### ✨ 多图截图

「+ 再截一张」最多 5 张；每张支持单独重新截图、重新标注、删除。提交端 adapters（禅道/webhook/cloud）+ 历史记录全链路支持多图。

### 🔴 表单草稿跨重挂真保留（`dialogDraft` 模块级单例）

旧版「再截一张/重新截图/重新标注」的 confirm 文案说「表单内容会保留」，实际弹窗卸载后标题/描述全丢 —— 文案撒谎。新增 `dialogDraft` 模块级单例：标题/描述/选项/已选元素/弹窗位置跨弹窗卸载重挂真保留。

### 🔴 fix(useFocusTrap)：恢复焦点从未生效

setup 期注册的 watcher 在 post 队列里抢跑在 v-show display 翻转之前，对仍 hidden 的容器调 `focus()` 静默 no-op —— 弹窗恢复后焦点一直没回来。修：`nextTick` 推迟 + 可见性双守卫。

### 📝 docs

全量文档脱敏修订；`check-version-consistency` 的 ⌘⇧B 快捷键对账改双向（文档↔源码互查）；mock-server 路径对齐协议文档。

### 测试

- **+40 单测 / +21 e2e**（dialogDraft round-trip、focusTrap、多图真流程、弹窗 UX e2e）。
- **808 单测（53 文件）+ 176 e2e 全绿**，e2e 跑两遍零 flake（含 CDP 真扩展驱动）；type-check + build 绿。

### 发版决策小记（跳 RELEASE_TEST_CHECKLIST 理由）

非 BREAKING + 全绿（808 单测 + 176 e2e 两遍零 flake）+ 用户明示「commit push, publish」放行。dogfood 不足（弹窗 UX/多图/草稿是这两天新做的）—— 留 dogfood 观察点：① 多图 + 草稿在真禅道实例提交链路 ② 弹窗拖拽/缩小在各类宿主页面（重 CSS 站点）的表现。
