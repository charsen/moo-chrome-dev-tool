...
- **content world 走 tokens.css**：`src/content/styles.ts` 重复定义了一套 `--c-brand` 等 token（因为 shadow DOM 隔离），跟 tokens.css 完全平行；如果 vite 能做 shadow root style 注入，可以收口到一处
- **悬浮球多 server 分组**：多个 server 同时匹配时悬浮球点击会跳"项目选择"小弹窗，列表长了不好看，可以加分组 / 搜索
