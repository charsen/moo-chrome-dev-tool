...
**v0.8.0 已发**（2026-05-27）。无 BREAKING — `/full-team-review` 3 expert agent 全仓审计修 **3 🔴 + 10 🟡 + 5 🟢**（无现网 bug，全是「未炸但明确」隐患）：content world payload 校验加严防 redact throw + offscreen state 6 直写改 transition() + permissions debounce 防 quota + storage.onChanged 兜底 offscreen 自停 + E2E harness release 排除 + v-for key 稳定 + shadow alias 桥补齐 + 死资产 1.36MB 清。+22 fuzz/编排单测，行为完全兼容。
