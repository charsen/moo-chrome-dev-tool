...
- **content 世界没 `<MooCloseBtn>` 等价**：当前 MooCloseBtn 是共享组件但 CSS 在两个世界各自定义，组件本身没问题。如果之后要做 content 世界的 toast / dialog 抽象，可以再起一个 `src/content/components/` 目录

**已划掉**：
- ~~**CI 接入 Playwright E2E**~~：commit `630d90f` 加了并行 `e2e` job（chromium `--with-deps` 装 + 跑 `pnpm test:e2e` + 失败 upload `test-results/` artifact 7 天）。⚠️ 仓库主在 gitee，workflow 仅在 mirror 到 github 时生效——本仓单 gitee 远端，CI 不会自跑；mirror 后即激活
- ~~**content world 走 tokens.css**~~：commit `1e26536` 用 vite `?raw` 法落地。`tokens.css` 顶层 `:root` 块单一来源，shadow `.moo-root` 嵌入复用，旧 `--c-*` 别名转 `--moo-c-*`。顺手修了两处 silent drift
- ~~**History tab 性能**~~：commit `6352841` 用 `content-visibility: auto` 解决，零 JS，零库依赖
- ~~**悬浮球多 server 分组**~~：判定不做（实际同时命中 N 个 server 的场景太少）
