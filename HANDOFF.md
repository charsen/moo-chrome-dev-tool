...
> v0.8.0 及更早一句话段见 [docs/handoff-archive/v0.5.x-v0.8.x.md](docs/handoff-archive/v0.5.x-v0.8.x.md)（v0.8.0）；v0.7.8 → v0.5.0 仍列于下方（历史堆积，下次清理批量归档）。
...
v0.8.2 已发完。**当前没有强迫性 todo**。本版是 `/full-team-review` 全仓 4 断面审计的全清（2🔴+2🟡+1🟢+helper 去重，无现网 bug、行为兼容），用户明示放行跳 dogfood + RELEASE_TEST_CHECKLIST。等用户继续真实 dogfood 反馈，再决定 hotfix 还是新 feature。
