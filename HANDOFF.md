...
**v0.8.17 已发**（2026-08-29）。[下载](https://gitee.com/charsen/moo-chrome-dev-tool/releases/download/v0.8.17/moo-chrome-dev-tool-0.8.17.zip)（sha256 `a318b998c412d94086298fb26b9b01998510bbcfa7b6c1e762af55a32b63a185`）。无 BREAKING；仅统一根目录协作文档命名为 `NOTES.md` 与 `TODOS.md`，不改变扩展权限、存储 schema、上报协议或运行时行为。详情见 CHANGELOG v0.8.17 段。

**v0.8.16 已发**（2026-07-05）。[下载](https://gitee.com/charsen/moo-chrome-dev-tool/releases/download/v0.8.16/moo-chrome-dev-tool-0.8.16.zip)（sha256 `c34c49b5187965ca8216670899e5e530d0d36bb29e6c60bb6d5f0cf475f7866e`）。无 BREAKING（纯键盘 / 交互守卫，不碰存储 schema / 上传协议 / 匹配引擎）— 🐛 **提交弹窗点遮罩不再关闭**：用户在标题/描述框按下拖选文本、手滑拖出弹窗到遮罩松手 → 浏览器把 `click` 派发到共同祖先（mask）→ 老 `@click.self` 误判「点 mask」→ cancel 丢草稿。改 `maskClosable=false`（`onMaskClick`→`onDialogClose`，收敛为 Esc/✕/取消三条主动退出；成功视图早返守卫保留）+ MooDialog 加防误关反馈（点 mask 播一次 0.22s 轻微 `scale` 脉冲 `.moo-dialog--bump`，用 `scale` 属性不撞拖拽 inline `translate` / 入场动画 `transform`、`animationend` 按名自清、`prefers-reduced-motion` 退化）。🐛 **中文输入法组字守卫**：组字中 Esc=取消候选、Enter=选字、Tab/方向键移候选，都不是命令，漏守卫 = 中文用户打字按 Esc 直接关窗丢内容 / 按 Enter 误提交。统一加 `if (e.isComposing) return`，覆盖全部 keydown 收口点（`useFocusTrap` / MooDialog 缩小态 / SubmitDialog / Annotator `onKey` + 文字标注 Enter/Esc / ElementPicker / ConfirmModal / PayloadEditorModal / Settings TagInput）。864 单测（含 isComposing 2 例）+ e2e（D3 改写「点 mask 不关 + bump 脉冲」+ 新增 UX11「跨界松手不关不丢」）+ type-check + build 全绿。**dogfood 不足（刚做），用户明示放行；IME 组字 / 点遮罩防误关无法 MCP 自动化（Playwright 合成不了 trusted composition + 跨界松手时序），真输入法路径留 checklist 22-23 人肉。留观：① 标题框拖选文本手滑到遮罩不再关窗丢草稿；② 点遮罩播脉冲、✕/Esc/取消仍正常关；③ 中文组字中 Esc 只取消候选、Enter 只选字（各 modal / TagInput 同款）。** 详情见 CHANGELOG v0.8.16 段。

**v0.8.15 已发**（2026-06-22）。[下载](https://gitee.com/charsen/moo-chrome-dev-tool/releases/download/v0.8.15/moo-chrome-dev-tool-0.8.15.zip)（sha256 `aff875253f90b53a05705bb54d51ee9d797da3837822f99e93680f38c20db976`）。无 BREAKING（不碰存储 schema / 上传协议）— ✨ **提交弹窗支持 ⌘V/Ctrl+V 粘贴图片**：弹窗内粘剪贴板图片 → 追加为一张截图，走截屏图同款降采样 / 上传链路；纯文本粘贴照常进输入框（仅图片 `preventDefault`），`MAX_SHOTS` 上限守卫 + 满额 toast；取图抽纯函数 `imageFileFromClipboard` + 8 单测。🐛 **修 `downscaleToMaxWidth` 撞宿主 CSP**：它现也在 content world 跑（粘贴图降采样），宿主 CSP `connect-src 'self'` 不含 `data:` 时 `fetch(dataUrl)` 会 Failed to fetch → 改 `atob` 同步解析（`dataUrlToBlob`）绕开，失败兜底返原图，SW 截图路径同样适用。🐛 **webhook 重试补 `images`**：`serializeForRetry` 漏 `images` 键，重试时默认模板 `{{imagesJson}}` 渲成非法 JSON → 小体积多图遇瞬时 5xx 重试丢图 / 坏 body，镜像 live submit 补齐 + 回归测试。862 单测（含 clipboardImage 8 例 + image / webhookAdapter 回归）+ e2e + type-check + build 全绿。**dogfood 不足（粘贴刚做），用户明示放行；剪贴板粘贴 e2e 无法 MCP 自动化（clipboard 注入受限），逻辑靠单测覆盖。留观：① 真弹窗 ⌘V 粘截图工具的图能追加并上传；② 纯文本粘贴不被吞；③ 满额 toast 不静默丢；④ 严格 CSP 宿主页粘贴图正常降采样；⑤ 小体积多图瞬时 5xx 重试不再丢图。** 详情见 CHANGELOG v0.8.15 段。
...
> **v0.8.12 及更早「一句话段」已批量归档**：v0.5.0 → v0.8.12 见 [docs/handoff-archive/v0.5.x-v0.8.x.md](docs/handoff-archive/v0.5.x-v0.8.x.md)；更早 v0.1.x → v0.4.x 见下方「早期版本简介」。本文「一句话现状」只保留最近 4 个发版（v0.8.13 → v0.8.16）。
