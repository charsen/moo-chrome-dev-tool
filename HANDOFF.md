...
> v0.8.3 及更早一句话段见 [docs/handoff-archive/v0.5.x-v0.8.x.md](docs/handoff-archive/v0.5.x-v0.8.x.md)（v0.8.0 / v0.8.1 / v0.8.2 / v0.8.3）；v0.7.8 → v0.5.0 仍列于下方（历史堆积，下次清理批量归档）。
...
v0.8.5 已发完。**当前没有强迫性 todo**。本版单一 **P0 修复**：版本检查升级后谎报旧版当新版（flag 缓存的 current 在用户升到「非被提示版本」后 stale → 读 flag 只查 age 不重比 → 谎报；修法新增 `readValidStoredVersionInfo()` 读取时用 live manifest 重比 + 覆盖 current + stale 清 flag，3 处内联校验收口）+ gitignore `.ccpanes`。无 BREAKING、行为兼容。这是版本检查显示逻辑 bug，**真实场景（升到非被提示版本）难手动 dogfood，但 7 单测 + e2e 覆盖充分（含核心 bug 场景），用户 review 后明示放行**（与 v0.8.2/v0.8.3 同款「明示放行跳」决策）。等用户继续真实 dogfood 反馈，再决定 hotfix 还是新 feature。
