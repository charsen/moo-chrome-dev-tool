...
v0.1.11 已发；v0.1.12 攒了 3 个 commit 在 master 待 release（染色升级 + Alt+Shift+M 开 popup + 两条小 UX fix）。v0.1.11 是上一版的架构纠偏：**token 从 header 移到 POST body，扩展退回 webhook 客户端的本分**。之前十个版本一直用 `Authorization: Bearer` + `X-Scaffold-Token` 两个 header，把扩展跟 scaffold 后端绑死了；其实 token 走 body 字段就够，跟 webhook 一样——扩展不该假设后端长啥样。**v0.1.11 是 BREAKING 变更**，v0.1.12 纯 UX 增量，详见 CHANGELOG.md。
...
测试覆盖：v0.1.12 已自动验证染色（headless chrome 截图核对）+ manifest 命令注册（dist/manifest.json）+ SW bundle 含 `openPopup()`；popup 快捷键已用户实测 OK。release 前过一遍 `docs/RELEASE_TEST_CHECKLIST.md` § v0.1.12 三条场景
