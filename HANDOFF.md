...
**v0.8.2 已发**（2026-05-29）。sha256 待回填。无 BREAKING — `/full-team-review` 全仓 4 断面审计全清：2🔴（SubmitDialog 浅 watch 改 slice 源修「开着期间新请求/错误不自动勾」+ options 暗色 hover 白字改 token）+ 2🟡（SW rehydrate race / retry 队列 enqueuedAt 唯一）+ 1🟢（裸 setTimeout 纳入 onBeforeUnmount）+ helper 去重（行为不变）。+4 e2e +1 单测。**用户明示放行跳 dogfood + checklist。**详情见 CHANGELOG v0.8.2 段。
