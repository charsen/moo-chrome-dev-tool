...
> v0.8.2 及更早一句话段见 [docs/handoff-archive/v0.5.x-v0.8.x.md](docs/handoff-archive/v0.5.x-v0.8.x.md)（v0.8.0 / v0.8.1 / v0.8.2）；v0.7.8 → v0.5.0 仍列于下方（历史堆积，下次清理批量归档）。
...
v0.8.4 已发完。**当前没有强迫性 todo**。本版一块新功能（标注器加「复制」按钮：合成图一键复制到剪贴板，仅安全上下文可用、HTTP 页置灰，免 chrome.downloads 权限）+ 一轮 full-team-review 简化（删死代码/死参数 + 压平嵌套三元 + 去重，行为不变）。无 BREAKING、行为兼容，**本版 dogfood 已验**（用户真机后明示「都好了」，三条标准齐 = 非 BREAKING + 全绿 + dogfood，正常跳 checklist，区别于 v0.8.2/v0.8.3 的「明示放行跳」）。等用户继续真实 dogfood 反馈，再决定 hotfix 还是新 feature。
