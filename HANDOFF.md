...
**v0.8.3 已发**（2026-05-30）。下载 + sha256 见 Gitee release 页（`pnpm release --publish` 后回填本段）。无 BREAKING，纯增量 — **截屏标注器加「下载」按钮**：标注后的图（背景截图 + 标注层合成 PNG）直接存浏览器下载目录。`composeCanvas()` 合成 → `toBlob` → `<a download>` 触发，**不走 chrome.downloads**（守最小权限），下载后不关界面可继续改 / 再提交。+3 e2e（真机验证 PNG 落盘）。**用户明示放行跳 dogfood + checklist（与 v0.8.2 同款决策）。**详情见 CHANGELOG v0.8.3 段。
