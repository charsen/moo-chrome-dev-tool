...
> **v0.8.5 及更早「一句话段」已批量归档**：v0.5.0 → v0.8.5 见 [docs/handoff-archive/v0.5.x-v0.8.x.md](docs/handoff-archive/v0.5.x-v0.8.x.md)；更早 v0.1.x → v0.4.x 见下方「早期版本简介」。本文「一句话现状」只保留最近 4 个发版（v0.8.6 → v0.8.9）。
...
> **最新 case 数看顶部「一句话现状」**（v0.8.9 时 155 case），本段只讲结构和原因，不再回填具体数字。
...
v0.8.9 已发完。**当前没有强迫性 todo**。v0.8.7 → v0.8.9 三连（多轮主动 review 累计 25+ 真 bug 全修 + 记账 8 项全清，详见 CHANGELOG）均无 BREAKING、全绿、用户明示放行。**留观的手测点**：普通账号禅道指派人下拉（v0.8.9 tier-3 兜底）+ 元素选取 hover。等用户继续真实 dogfood 反馈，再决定 hotfix 还是新 feature。
